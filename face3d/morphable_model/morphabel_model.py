...
class MorphabelModel(object):
    ...
    # --------------------------------------------------- fitting
    def fit(self, x, X_ind, face_colors, max_iter=4, isShow=False):
        ...
        """
        if isShow:
            fitted_sp, fitted_ep, fitted_tp, s, R, t = fit.fit_points_for_show(x, X_ind, face_colors, self.model,
                                                                               n_sp=self.n_shape_para,
                                                                               n_ep=self.n_exp_para,
                                                                               n_tp=self.n_tex_para, max_iter=max_iter)
            angles = np.zeros((R.shape[0], 3))
            for i in range(R.shape[0]):
                angles[i] = mesh.transform.matrix2angle(R[i])
        else:
            fitted_sp, fitted_ep, fitted_tp, s, R, t = fit.fit_points(x, X_ind, face_colors, self.model,
                                                                      n_sp=self.n_shape_para, n_ep=self.n_exp_para,
                                                                      n_tp=self.n_tex_para, max_iter=max_iter)
            angles = mesh.transform.matrix2angle(R)
        return fitted_sp, fitted_ep, fitted_tp, s, angles, t
