...
def estimate_tex(x, texMU, texPC, texEV, tex, s, R, t2d, lamb=2000):
    """
    Args:
        x: (2, n). image points (to be fitted)
        texMU: (3n, 1)
    ...
    """
    x = x.copy()
    assert (texMU.shape[0] == texPC.shape[0])
    assert (texMU.shape[0] == x.shape[1] * 3)
    ...
    # --- calc pc
    pc = texPC
    # pc_3d = np.resize(texPC.T, [dof, n, 3])
    # pc_3d = np.reshape(pc_3d, [dof * n, 3])
    # pc_2d = pc_3d.dot(A.T)
    # pc = np.reshape(pc_2d, [dof, -1]).T  # 2n x 29
    ...
    # --- calc b
    # shapeMU
    mu_3d = np.resize(texMU, [n, 3]).T  # 3 x n
    # expression
    # shape_3d = shape
    # b = A.dot(mu_3d + shape_3d) + np.tile(t2d[:, np.newaxis], [1, n])  # 2 x n
    b = mu_3d + tex
...
# ---------------- fit 
def fit_points(x, X_ind, face_colors, model, n_sp, n_ep, n_tp, max_iter=4):
    """
    Args:
        x: (n, 2) image points
    ...
    # MU:average 160470(53490*3)*1
    # EV:variance 199*1
    # PC:principal component 160470(53490*3)*199
    shapeMU = model['shapeMU'][valid_ind, :]
    shapePC = model['shapePC'][valid_ind, :n_sp]
    expPC = model['expPC'][valid_ind, :n_ep]
    texMU = model['texMU'][valid_ind, :]
    texPC = model['texPC'][valid_ind, :n_tp]
    texEV = model['texEV']
    ...
    for i in range(max_iter):
        ...
        new_ep = estimate_expression(x, shapeMU, expPC, model['expEV'][:n_ep, :], shape, s, R, t[:2], lamb=20)
        ep = (1 - lr) * ep + lr * new_ep

        # shape
        expression = expPC.dot(ep)
        expression = np.reshape(expression, [int(len(expression) / 3), 3]).T
        new_sp = estimate_shape(x, shapeMU, shapePC, model['shapeEV'][:n_sp, :], expression, s, R, t[:2], lamb=40)
        sp = (1 - lr) * sp + lr * new_sp

        # texture
        tex = texPC.dot(tp)
        tex = np.reshape(tex, [int(len(tex) / 3), 3]).T
        new_tp = estimate_tex(face_colors, texMU, texPC, model['texEV'][:n_tp, :], tex, s, R, t[:2], lamb=20)
        tp = (1 - lr) * tp + lr * new_tp

    return sp, ep, tp, s, R, t


# ---------------- fitting process
def fit_points_for_show(x, X_ind, face_colors, model, n_sp, n_ep, n_tp, max_iter=4):
    '''
    Args:
        x: (n, 2) image points
        X_ind: (n,) corresponding Model vertex indices
    ...
    sp = np.zeros((n_sp, 1), dtype=np.float32)
    ep = np.zeros((n_ep, 1), dtype=np.float32)
    tp = np.zeros((n_sp, 1), dtype=np.float32)
    ...
    expPC = model['expPC'][valid_ind, :n_ep]
    texMU = model['texMU'][valid_ind, :]
    texPC = model['texPC'][valid_ind, :]

    s = 4e-04
    R = mesh.transform.angle2matrix([0, 0, 0])
    t = [0, 0, 0]
    lsp, lep, ltp, ls, lR, lt = [], [], [], [], [], []

    for i in range(max_iter):
        X = shapeMU + shapePC.dot(sp) + expPC.dot(ep)
        X = np.reshape(X, [int(len(X) / 3), 3]).T

        lsp.append(sp), lep.append(ep), ltp.append(tp)
    ...
    # print('ls', ls)
    # print('lR', lR)
    return np.array(lsp), np.array(lep), np.array(ltp), np.array(ls), np.array(lR), np.array(lt)
